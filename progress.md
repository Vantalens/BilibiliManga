...
- 已初始化本地 codegraph 索引：44 个文件、534 个符号、1207 条边。
- 跨模块修改前应优先使用 codegraph 定位影响面；Rust 细节仍需结合源码读取确认。
...
- 已修正阅读器章节图片加载路径：只有本机没有 Cookie 时才尝试匿名请求；带 Cookie 的官方接口失败会原样暴露给用户，避免隐藏真实登录态/API 问题。
- 已新增“我的账号”页阅读诊断：可一键检查登录状态、书架同步、应用内搜索、漫画详情和章节图片加载，结果脱敏显示，用于真实账号验收定位。
- 已新增脱敏阅读诊断报告：诊断完成后可复制文本报告，包含应用版本、平台和各步骤结果，不包含 Cookie、UID、图片鉴权 URL 或 token。
